namespace BLL.Services.Impelementation
{
    public class IdentityService : IIdentityService
    {
        ...
        public async Task<Response<string>> LoginWithFaceAsync(string base64Image)
        {
            try
            {
                // Verify face and get user ID
                var verifyResult = await _faceRecognitionService.VerifyFaceAsync(base64Image);

                if (!verifyResult.Success)
                    return Response<string>.FailResponse(verifyResult.errorMessage ?? "Face not recognized");

                // Get user
                var userId = Guid.Parse(verifyResult.result!);
                var user = await _userManager.FindByIdAsync(userId.ToString());

                if (user == null || !user.IsActive)
                    return Response<string>.FailResponse("User account is not active");
            ...
            {
                return Response<string>.FailResponse($"Login failed: {ex.Message}");
            }
        }
